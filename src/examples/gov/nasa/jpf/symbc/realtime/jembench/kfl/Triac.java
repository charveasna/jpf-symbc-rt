...
public class Triac {
	...
	private static final int OPTO_LEN = 100;		// opto_idx with mask 0x03!
...
// default Werte
		...
		impVal = Debug.makeSymbolicInteger("Symbolic" + symId++) & BBSys.BIT_SENSI;//JopSys.rd(BBSys.IO_SENSOR) & BBSys.BIT_SENSI;
	}

	private static int symId = 1;
...
*/
	private static void doOpto() {
		...
		i = opto_idx;
		opto[i] = Debug.makeSymbolicInteger("OPTO SYM" + symId++);//JopSys.rd(BBSys.IO_TRIAC);
		++i;
		i &= 0x03;
		opto_idx = i;
...
*/
	...
	private static void doSensor() {
		...
		// impuls, end sensors
		sens = Debug.makeSymbolicInteger("SENSSYM" + symId++);//JopSys.rd(BBSys.IO_SENSOR);
