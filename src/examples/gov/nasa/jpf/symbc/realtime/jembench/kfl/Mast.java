...
public class Mast extends PeriodicThread {

	public Mast(PeriodicParameters pp) {
		super(pp);
...
*/
	static boolean handleMsg(int val) {
		...
		if (val==BBSys.CMD_INP) {
			Msg.write(Debug.makeSymbolicInteger("WRITESYM" + symId++));//(JopSys.rd(BBSys.IO_SENSOR)<<4) + JopSys.rd(BBSys.IO_TAST));
			return true;
		}
